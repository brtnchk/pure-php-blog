...
- `ArticleService` — нормализация sort, расчёт пагинации (зажим page
  сверху и снизу, пустой результат), инкремент просмотров, бандл для
  страницы статьи.
- `CategoryService` — секции для главной, бандл для страницы категории,
  null для несуществующего slug.
- `Container` — мемоизация, автоваринг графа из одного PDO-биндинга,
